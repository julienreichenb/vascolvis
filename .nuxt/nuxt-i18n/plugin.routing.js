...
function localePath(route, locale) {
  ...
  if (strategy === STRATEGIES.NO_PREFIX && locale && locale !== i18n.locale) {
    ...
    console.warn(
      `[${MODULE_NAME}] Passing non-current locale to localePath is unsupported when using no_prefix strategy`
    )
  }
  ...
  const {
    route: { fullPath }
  } = this.router.resolve(localizedRoute)
  return fullPath
}
...
function switchLocalePath(locale) {
  ...
  if (strategy === STRATEGIES.NO_PREFIX && locale && locale !== i18n.locale) {
    // eslint-disable-next-line no-console
    console.warn(
      `[${MODULE_NAME}] Passing non-current locale to switchLocalePath is unsupported when using no_prefix strategy`
    )
  }
